...
# CSV Dosyası Yükleme
filename = r'C:\Users\BUŞRA\Desktop\all\sonuc\counter\xxlcmix.csv'
...
from sklearn.neighbors import KNeighborsClassifier
estimator = []
estimator.append(('Neural Network', MLPClassifier(hidden_layer_sizes=(100,), activation='relu', solver='adam', max_iter=10000)))
estimator.append(('LogisticRegression', LogisticRegression(solver='lbfgs', max_iter=200)))
estimator.append(('ExtraTreesClassifier', ExtraTreesClassifier(n_estimators=100, criterion='gini', max_features='sqrt')))
estimator.append(('RandomForest',RandomForestClassifier(n_estimators=150, max_depth=20, random_state=42)))
estimator.append(('KNN', KNeighborsClassifier(n_neighbors=5, metric='minkowski', p=2)))
estimator.append(('SVC', SVC(kernel='rbf', gamma='scale', C=1.0, probability=True)))
estimator.append(('AdaBoostClassifier', AdaBoostClassifier(n_estimators=50, learning_rate=1.0)))
estimator.append(('GradientBoostingClassifier', GradientBoostingClassifier(n_estimators=100, learning_rate=0.1, max_depth=3)))
estimator.append(('XGB', XGBClassifier()))
estimator.append(('CatBoost', CatBoostClassifier(iterations=500, depth=6, learning_rate=0.1, logging_level='Silent')))

# Voting Classifier kullanımı (soft voting önerilir)
rf_model = VotingClassifier(estimators=estimator, voting='soft')

#rf_model = VotingClassifier(estimators = estimator, voting ='hard')
